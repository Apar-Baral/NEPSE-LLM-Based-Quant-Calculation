...
p = Path(__file__).resolve().parent.parent / "frontend_react" / "src" / "pages" / "SymbolDetail.jsx"
lines = [
    ...
    '      </select>',
    '      {latest.symbol && (',
    '        <motion className="metrics">',
    '        <div className="metrics">',
    '          <div className="metric"><div className="label">Tier</div><div className="value">{latest.signal_tier}</div></div>',
    '          <div className="metric"><div className="label">P(Long)</div><motion className="value">{((latest.p_long_momentum || 0) * 100).toFixed(0)}%</motion><div className="value">{((latest.p_long_momentum || 0) * 100).toFixed(0)}%</div></div>',
